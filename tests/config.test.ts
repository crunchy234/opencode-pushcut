...
describe("parsePushcutBackendConfig", () => {
  ...
  it("should throw when fetchTimeout is not a string", () => {
    expect(() =>
      parsePushcutBackendConfig({ notificationName: "MyNotif", apiKey: "abc123", fetchTimeout: 9999 })
    ).toThrow("fetchTimeout");
  });
  it("should throw when title is not an object", () => {
    expect(() =>
      parsePushcutBackendConfig({ notificationName: "MyNotif", apiKey: "abc123", title: "bad" })
    ).toThrow("title");
  });
  it("should throw when message is not an object", () => {
    expect(() =>
      parsePushcutBackendConfig({ notificationName: "MyNotif", apiKey: "abc123", message: "bad" })
    ).toThrow("message");
  });
